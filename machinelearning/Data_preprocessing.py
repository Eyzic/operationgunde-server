...
def make_dataset_for_NN(name__,days_sequence ,start_day,end_day,hrv_excel_path,training_excel_path,X_columns,Y_columns):
  ...
  scaler_Y = MinMaxScaler(feature_range=(0,1)).fit(d[Y_columns])

  scaler_X = MinMaxScaler(feature_range=(0,1)).fit(d[X_columns])
  ...
  for i in range(len(list_dataframes)):
    ...
    Y_data_frame = np.asarray(list_dataframes[i][Y_columns]).astype ('float64')
    X_data_frame = np.asarray(list_dataframes[i][X_columns]).astype ('float64')
    for j in range (start_day,end_day):
      X_train.append(X_data_frame[j-days_sequence:j])
...
def last_day_index (person,hrv_excel_path,training_excel_path):
  ...
  # Make class for frames
  class frame_class:
    def __init__(self, date, name, sleep, stress, muscle_pain, humor, injuries, energi_level, training_load, RMSSD,
                 SDNN, HRV_value):
      self.date = date
      self.name = name
      self.sleep = sleep
  ...
  for i in range(hrv.shape[0]):
    ...
    HRV_value_ = hrv['HRV'][i]
    training_load_ = float('NaN')
    fr_ = frame_class(date_, name_, sleep_, stress_, pain_, humor_, injuries_, energy_, training_load_, RMSSD_, SDNN_,
                      HRV_value_)
    frame_list_hrv.append(fr_)
  frame_list_training = []
  for i in range(training.shape[0]):
    date_ = datetime.date(int(training['Tidstämpel'][i].split()[0].split('-')[0]),
                          int(training['Tidstämpel'][i].split()[0].split('-')[1]),
                          int(training['Tidstämpel'][i].split()[0].split('-')[2]))
    date_ = date_ + datetime.timedelta(days=1)
    name_ = training['Namn:'][i]
    sleep_ = float('NaN')
    stress_ = float('NaN')
    ...
    SDNN_ = float('NaN')
    HRV_value_ = float('NaN')
    if training['Hur länge varade träningspasset (i min):'][i] != training['Hur länge varade träningspasset (i min):'][
      i]:
      continue
    if training['Hur länge varade träningspasset (i min):'][i] == 0:
      training_load_ = 0
    if training['Hur länge varade träningspasset (i min):'][i] != 0:
      training_load_ = training['Hur intensiv kände du att träningen var?'][i] * \
                       training['Hur länge varade träningspasset (i min):'][i]
    fr_ = frame_class(date_, name_, sleep_, stress_, pain_, humor_, injuries_, energy_, training_load_, RMSSD_, SDNN_,
                      HRV_value_)
    frame_list_training.append(fr_)
