...
d_Christer = pd.DataFrame(columns=Columns)
d_Einar = pd.DataFrame(columns=Columns)
d_Thomas_ = pd.DataFrame(columns=Columns)
d_Jessica = pd.DataFrame(columns=Columns)
d_Filip = pd.DataFrame(columns=Columns)
d_Jonny = pd.DataFrame(columns=Columns)

for x in range(d.shape[0]):

    if d['name'][x] == 'ch':
        d_Christer = d_Christer.append(pd.DataFrame(d.iloc[[x]], columns=Columns), ignore_index=True)

    if d['name'][x] == 'ei':
        d_Einar = d_Einar.append(pd.DataFrame(d.iloc[[x]], columns=Columns), ignore_index=True)

    if d['name'][x] == 'th':
        d_Thomas_ = d_Thomas_.append(pd.DataFrame(d.iloc[[x]], columns=Columns), ignore_index=True)

    if d['name'][x] == 'je':
        d_Jessica = d_Jessica.append(pd.DataFrame(d.iloc[[x]], columns=Columns), ignore_index=True)

    if d['name'][x] == 'fi':
        d_Filip = d_Filip.append(pd.DataFrame(d.iloc[[x]], columns=Columns), ignore_index=True)

    if d['name'][x] == 'jo':
        d_Jonny = d_Jonny.append(pd.DataFrame(d.iloc[[x]], columns=Columns), ignore_index=True)

# Make dataset compatible for RNN
days_sequence = 7
percentage_of_training_examples = 0.8
list_dataframes = [d_Jessica,d_Christer,d_Einar,d_Filip,d_Jonny,d_Thomas_]
...
for i in range(len(list_dataframes)):
  ...
  for j in range (days_sequence,int((data_frame_.shape[0]-7)*percentage_of_training_examples+days_sequence)):
...
for i in range(len(list_dataframes)):
  ...
  Y_data_frame = np.asarray(list_dataframes[i]['HRV']).astype ('float64')
  X_data_frame_ = np.asarray(data_frame_.values).astype('float64')
  for j in range (int((data_frame_.shape[0]-7)*percentage_of_training_examples+days_sequence),data_frame_.shape[0]):
    X_test.append(X_data_frame_[j-days_sequence:j])
    Y_test.append(Y_data_frame[j])
